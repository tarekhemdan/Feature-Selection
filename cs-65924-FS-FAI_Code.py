...
def collect_features(df_train):
    ...
    for index, row in df_train.iterrows():
      if df_train.get_value(index,'labels')=='business':
          b=[]
      ...
      elif df_train.get_value(index,'labels')=='tech':
          t=[]
          for c in df_train.columns[0:l-1]:
               if df_train.get_value(index,c)!=0:
                      t.append(c)
          tech.append(t)        
      elif df_train.get_value(index,'labels')=='sport':
          s=[]
          for c in df_train.columns[0:l-1]:
               if df_train.get_value(index,c)!=0:
                      s.append(c)
          sport.append(s)
      elif df_train.get_value(index,'labels')=='entertainment':
          e=[]
          for c in df_train.columns[0:l-1]:
               if df_train.get_value(index,c)!=0:
                      e.append(c)
          entertainment.append(e)
      else:
          po=[]
          for c in df_train.columns[0:l-1]:
               if df_train.get_value(index,c)!=0:
                      po.append(c)
          politics.append(po)
    return  business,tech,sport,entertainment,politics
...
def fit_features(df_train,features_b,features_s,features_t,features_en,features_po):
...
def main():    
    ...
    df_bbc = pd.read_csv('E:\\phd thesis\\papers_exper\\paper 3\\bbctext.csv',encoding="utf-8")
...
#  4- tf_idf   
   ...
   #  5-  collect features from train dataframe in each class && convert it to transactions
    ...
    features_b=fs_ass(business)
    
    features_t=fs_ass(tech)
    
    features_s=fs_ass(sport)
    
    features_en=fs_ass(entertainment)
    
    features_po=fs_ass(politics)
   
    
    total_features=len(features_b)+len(features_t)+len(features_s)+ len(features_en)+len(features_po)
    ...
    df_train= fit_features(df_train,features_b,features_s,features_t,features_en,features_po)
    df_test =fit_features(df_test ,features_b,features_s,features_t,features_en,features_po)
